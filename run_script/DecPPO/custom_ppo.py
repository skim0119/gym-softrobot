...
class CustomPPO(PPO):
    def __init__(self, n_agent=5, n_duplicate_data=1, progress_bar=True, **kwargs):
        self.n_agent = n_agent
        self.n_duplicate_data = n_duplicate_data
    ...
    def _setup_model(self) -> None:
        ...
        self.rollout_buffer = buffer_cls(
            ...
            n_envs=self.n_envs*self.n_duplicate_data,
        )
    ...
    def collect_rollouts(
        ...
        n_steps = 0
        env_num_envs = env.num_envs * self.n_duplicate_data
        ...
        while n_steps < n_rollout_steps:
            ...
            for key, space in self.observation_space.spaces.items():
                obs[key] = np.reshape(self._last_obs[key], [env_num_envs]+list(space.shape))
            # obs = np.reshape(self._last_obs, [env_num_envs]+list(self.observation_space.shape))
            with th.no_grad():
            ...
            reshaped_actions = np.reshape(clipped_actions, [env.num_envs,self.n_duplicate_data]+list(self.action_space.shape))
            try:
            ...
            dones = np.reshape([info['done'] for info in infos], [-1])
        ...
        # Reshape spaces
        obs = {}
        for key, space in self.observation_space.spaces:
            obs[key] = np.reshape(self._last_obs[key], [env_num_envs]+list(space.shape))
        with th.no_grad():
