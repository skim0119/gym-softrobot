...
from gym_softrobot.utils.actuation.actuations.muscles.apply_muscles import ApplyMuscle
...
class ArmPushEnv(core.Env):
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(
            action_low, action_high, shape=(n_action), dtype=np.float32
        )
        self._observation_size = (
            (self.n_seg + (self.n_elems + 1) * 4 + n_action + 2),
        )  # 2 for target
    ...
    def step(self, time, muscle_activations):
        ...
        for muscle_count in range(len(self.muscle_layers)):
            self.muscle_layers[muscle_count].set_activation(
                muscle_activations[muscle_count] * scale
            )
