...
def generate_trajectory(final_time, sim_dt, target_v_scale):
    end_time = final_time * 1.1 #Use of 1.1 just adds a little buffer on the end of the trajectory.
    ...
    t += np.random.rand(1)[0] * 3600 #start the trajectory anywhere within a 1 hour period

    f1 = np.random.uniform(2,5)*.025 * target_v_scale
    f2 = np.random.uniform(2,5)*.025 * target_v_scale
    f3 = np.random.uniform(2,5)*.025 * target_v_scale
    direction = np.random.randint(0,1) * 2 - 1
    target_trajectory[:,0] = direction * 0.8*np.sin(2*np.pi*f1 * t)*np.sin(2*np.pi*f2 * t)*np.sin(2*np.pi*f3 * t) * 1000

    f1 = np.random.uniform(2,5)*.025 * target_v_scale
    f2 = np.random.uniform(2,5)*.025 * target_v_scale
    f3 = np.random.uniform(2,5)*.025 * target_v_scale
    direction = np.random.randint(0,1) * 2 - 1
    target_trajectory[:,1] = direction * 0.4*np.sin(2*np.pi*f1 * t)*np.sin(2*np.pi*f2 * t)*np.sin(2*np.pi*f3 * t) * 1000
    ...
    f3 = np.random.uniform(2,5)*.025 * target_v_scale
    direction = np.random.randint(0,1) * 2 - 1
    target_trajectory[:,2] = direction * 0.8*np.sin(2*np.pi*f1 * t)*np.sin(2*np.pi*f2 * t)*np.sin(2*np.pi*f3 * t) * 1000
...
class SoftArmTracking(gym.Env):
    ...
    def get_state(self):
        ...
        state = np.concatenate(
            (
                ...
                avg_kappa_1 * self.base_length / (2 * np.pi), #normalize by the curvature of if the rod is a perfect ring. 
                avg_kappa_2 * self.base_length / (2 * np.pi), #normalize by the curvature of if the rod is a perfect ring. 
                # arm tip location
    ...
    def step(self, action):
        ...
        """ Reward Engineering """
        tip_to_target = (self.sphere.position_collection[..., 0] - self.shearable_rod.position_collection[..., -1])/1000
    ...
    def reset(self):
            ...
            class ArmCallBack(CallBackBaseClass):
                ...
                def make_callback(self, system, time, current_step: int):
                    if current_step % self.every == 0:
                        ...
                        self.callback_params["omega_collection"].append(system.omega_collection.copy())
                        self.callback_params["sigma"].append(system.sigma.copy())
                        self.callback_params["tangents"].append(system.tangents.copy())
                        self.callback_params["velocity_collection"].append(system.velocity_collection.copy())
                        self.callback_params["acceleration_collection"].append(system.acceleration_collection.copy())
                        return
            # Collect data using callback function for postprocessing
            self.post_processing_dict_rod = defaultdict(list)
            self.simulator.collect_diagnostics(self.shearable_rod).using(ArmCallBack,
        ...
        target_trajectory = generate_trajectory(self.max_episode_final_time, self.sim_dt, self.target_v_scale)
        ...
        if self.COLLECT_DATA_FOR_POSTPROCESSING:
            ...
            self.simulator.collect_diagnostics(self.sphere).using(RigidSphereCallBack,
                step_skip=self.step_skip,callback_params=self.post_processing_dict_sphere,)
