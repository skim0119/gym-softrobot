...
class ReachEnv(Env):
    ...
    def __init__(
        ...
        self._observation_size = (
            n_arm
            * (self.n_seg + (self.n_elems + 1) * 4 + n_action + n_arm + shared_space),
        )
    ...
    def reset(
        ...
        *,
        ...
        self.shearable_rods, self.rigid_rod, self.muscle_activations = (
        ...
        self._target = self.np_random.random(3) * sum(
            self.shearable_rods[0].rest_lengths
        )
    ...
    def step(self, action):
        ...
        if invalid_values_condition == True:
            ...
            all_tip_pos = [
                self.shearable_rods[i].position_collection[:, -1]
                for i in range(self.n_arm)
            ]
            distance_all_tip_to_target = np.linalg.norm(
                self._target - all_tip_pos, axis=1
            )
